...
function Grid() {
    ...
    c.setupWindow = function() {
        ...
        if (c.settings.setupEnabled) {
            ...
            for (var d = 2; d < 48; d++) {
                if (Math.round((c.settings.size - (c.settings.gWidth * 2 * (d - 1)))  / d) === (c.settings.size - (c.settings.gWidth * 2 * (d - 1))) / d) {
    ...
    c.createGrid = function() {
        ...
        if (c.settings.gEnabled && c.settings.gColumns > 0) {
            c.settings.gColWidth = (c.settings.size - ((c.settings.gColumns-1) * 2 * c.settings.gWidth)) / c.settings.gColumns;
            if (c.settings.invert) {
                ...
                for (var g = 1; g < c.settings.gColumns; g++) {
                //for (var g = 0; g <= c.settings.gColumns; g++) {
                    c._createEntity("vertical", {
                        left: (g * c.settings.gColWidth) + ((g-1) * 2 * c.settings.gWidth),
                ...
                for (var g = 0; g < c.settings.gColumns; g++) {
                    ...
                    c._createEntity("vertical", {
                        left: (g * c.settings.gColWidth) + (g * c.settings.gWidth * 2) ,
                        width: c.settings.gColWidth + "px",
                        height: c.settings.height,
                        backgroundColor: c.settings.gColor,
                        opacity: c.settings.gOpacity
                    });
